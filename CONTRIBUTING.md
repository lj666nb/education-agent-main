...
## 2026-07-15 - 修复：AI 生成中切换会话被打断、消息保存失败

### 背景

用户反馈：AI 正在流式生成回复时，切换到其它对话会中断生成，消息只保存了截断内容并追加"（已停止生成）"。根因是 `ChatPlatform.handleSelectChat` 在检测到有会话正在发送时显式调用 `abortRef.current?.abort()` 中断流式请求（原意是避免切换竞态导致闪烁）。

### 变更

- `frontend/src/store/chat.ts`
  - 全局 `isLoading`/`setIsLoading` 改为按会话的 `loadingByChat`/`setChatLoading`，加载状态与 `messagesByChat` 一致按 chatId 隔离。
  - `resetChat`/`resetAll` 同步清理对应加载状态。
- `frontend/src/components/ChatPlatform.tsx`
  - 单个 `abortRef`/`sendingRef`/`sendingChatIdRef` 改为按会话的 `Map<chatId, AbortController>` 与 `Set<chatId>`，各会话生成互不干扰。
  - `handleSelectChat` 不再中断生成：切换会话时后台流式请求继续，完成后照常 `saveMessage` 持久化；切回正在生成的会话时直接展示 store 中的实时流式内容，不从后端重载（避免覆盖未保存的流式消息）。
  - `handleNewChat` 不清除正在生成会话的消息缓存。
  - "停止生成"按钮只中断当前查看会话；回滚重发同步清理该会话状态并带所有权校验，避免旧请求的清理误伤新请求。
  - 后台生成的 UI 副作用（推荐问题、无关内容警告、图表面板 XML）仅在用户正在查看该会话时生效，防止污染其他会话界面。
  - 删除死代码 `messagesRef`（对话历史改为发送时从 store 实时读取，同时修复多轮对话历史陈旧问题）；修复停止生成标记中错误的字面量 `\n\n`。
- `request/prd-4-AIChat.md`
  - P0_chat_history 记录用户反馈与修复说明。

### 验证

- TypeScript `tsc --noEmit` 通过；已执行 `docker-compose up -d` 与 `docker-compose restart frontend`。
- 新增 `test_script/e2e-background-generation.js`：Playwright 以 guoketg/123456 登录，会话 A 生成中切到会话 B、切回 A（实时流式内容可见）、再切走等待后台完成，验证 assistant 消息保存请求返回 200；刷新页面后从数据库重新加载，回复完整、无"（已停止生成）"标记。12 项断言全部通过。
- 截图：`test_script/screenshot-bg-gen-1-A-streaming.png` ~ `screenshot-bg-gen-5-A-after-reload.png`。
