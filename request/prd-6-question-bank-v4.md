...
| F20 | 数据结构代码练习与服务端 OJ | P0 | ✅ 是 | 🔴 否 | 公开运行和隐藏提交均提示未配置测试用例 | 已修复完整种子未同步判题用例的问题；21 题共 84 条公开/隐藏用例已恢复，浏览器端公开 2/2、全部 4/4 判题通过，等待用户复验 |
...
```json
{
  "ai_status": {
    ...
    "F20_coding_oj": {
      ...
      "completed": true,
      "passed": false,
      "user_feedback": "所有编程题运行公开测试和提交隐藏测试均显示判题请求失败，提示本题尚未配置测试用例",
      "notes": "已统一基础/完整种子的编程题数据入口，始终从 OJ 目录同步题面与服务端用例；数据库 21 题共 84 条用例，每题 2 条公开、2 条隐藏；单元回归与浏览器真实判题（公开 2/2、全部 4/4）通过，等待用户复验"
    }
  }
}
```
